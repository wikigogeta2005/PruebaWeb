...
function showAlert(message, isError = false) {
    if (!alertBox)
        return;
    alertBox.className = `p-4 rounded-xl text-sm font-medium border mb-4 ${isError
...
async function fetchPostById() {
    ...
    if (!id)
        return showAlert('Por favor introduce un ID válido', true);
...
async function fetchPostsByUserId() {
    ...
    if (!userId)
        return showAlert('Por favor introduce un ID de usuario válido', true);
...
async function createPost(e) {
    e.preventDefault();
    const userIdVal = parseInt(document.getElementById('post-user-id').value);
    ...
    try {
        ...
        if (!response.ok)
            throw new Error('No se pudo simular la creación');
        const data = await response.json();
...
async function updatePostTitle() {
    ...
    if (!newTitle)
        return showAlert('El título no puede estar vacío', true);
...
async function deletePost(id) {
    if (!confirm(`¿Estás seguro de que deseas eliminar el post #${id}?`))
        return;
    ...
    try {
        ...
        if (!response.ok)
            throw new Error('Error al simular borrado');
...
function openEditModal(id, currentTitle) {
    if (!editPostId || !editTitleInput || !editModal)
        return;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    searchIdInput = document.getElementById('search-id-input');
    filterUserInput = document.getElementById('filter-user-input');
